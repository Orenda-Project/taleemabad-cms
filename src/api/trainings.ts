...
export const getTrainings = (courseUuid: string) =>
  apiClient.get<Training[]>(`/api/v1/trainings/?is_active=null&course__uuid=${courseUuid}`).then(r => ensureArray<Training>(r.data))

export const createTrainings = (data: Record<string, unknown>[]) =>
  apiClient.post<Training[]>("/api/v1/internal/trainings/", data).then(r => ensureArray<Training>(r.data))
